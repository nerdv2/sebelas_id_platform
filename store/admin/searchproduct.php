...
$no = $posisi + 1;
$sql = 'select * from product_data where '.$searchwhat.'="'.$searchname.'" limit '.$posisi.','.$batas;
	...
	<tr>
		...
		<td colspan='11'><select name="searchwhat">
			<option selected="selected">--Search--</option>
				...
				<option name='id' value='category'>Category ID</option>
			</select>
...
<tr>
	<td colspan="9">
		<?php
		$query2 = mysqli_query($db_link,"select * from product_data where $searchwhat=$searchname");
		$jmldata    = mysqli_num_rows($query2);
		$jmlberanda = ceil($jmldata/$batas);
		//echo "<br> beranda : ";
		for($i=1;$i<=$jmlberanda;$i++)
		if ($i != $page){
			echo " <a href=\"searchproduct.php?page=$i\">$i</a> | ";
		}else{ 
			echo " | $i | "; 
		}
